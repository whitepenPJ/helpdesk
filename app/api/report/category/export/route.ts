...
export async function GET(request: Request): Promise<Response> {
    ...
    sections: [
      {
        ...
        rows: data.rows.map((r) => [r.categoryName, r.active, r.processing, r.done, r.total, slaDaysLabel(r.avgSlaDays)]),
  ...
  if (format === "pdf") {
    ...
    return new Response(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="category-report-${filenameDate}.pdf"`,
      },
    });
  }
  ...
  return new Response(new Uint8Array(buffer), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="category-report-${filenameDate}.xlsx"`,
    },
  });
}
