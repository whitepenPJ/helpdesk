...
// Bootstrap JS (with Popover) loads globally via the dashboard layout's own
// VendorScripts, not a script tag this component owns — so init has to poll
// for window.bootstrap rather than hooking a Script onLoad, same pattern as
// Select2Select waiting on window.jQuery.
declare global {
  interface Window {
    bootstrap?: {
      Popover: new (el: Element, options?: Record<string, unknown>) => { dispose: () => void };
    };
  }
}
...
const NEED_APPROVAL_HELP = "Sends this ticket straight to the department's approver before anyone else acts on it.";

function NeedApprovalInfoIcon() {
          ...
          <div className="row g-3">
            ...
            <div className="col-md-6">
              ...
              {now ? (
                ...
                <input type="text" className="form-control" defaultValue="Loading…" disabled readOnly aria-hidden="true" />
              )}
            ...
            <div className="col-12">
              <div className="form-check">
                ...
                <label className="form-check-label" htmlFor="needApproval">
                  Need approval
                  <NeedApprovalInfoIcon />
                </label>
                <div className="form-text">
                  {hasApprover ? (
                    "Approver" + (approvers.length > 1 ? "s" : "") + ": " + approvers.map((a) => `${a.name} (${a.email})`).join(", ")
                  ) : (
            ...
            {needApproval && hasApprover && (
              <div className="col-12">
                <label htmlFor="approvalMessage" className="form-label">
                  Message to approver
                </label>
                <textarea
                  id="approvalMessage"
                  ...
                  className="form-control"
                  rows={3}
                  defaultValue={values?.approvalMessage}
                  required
                />
                <FieldError messages={state?.errors?.approvalMessage} />
              </div>
            )}
