...
function historyIcon(action: string): { icon: string; color: string } {
  switch (action) {
    ...
    case "Approver Rejected":
      return { icon: "bi-x-circle-fill", color: "dark" };
    default:
...
export async function TicketDetailContent({
  ...
  /** The plain Tickets list's own View/Edit split — View links here with no
   * `edit` param (comments read-only), Edit adds `?edit=1` (comment box
   * enabled) — the only thing that differs between the two on this route.
   * Everywhere else the comment box shows unconditionally regardless of
   * `edit`. */
  commentGate?: boolean;
  /** Ticket Management's own View/Edit split — View (no `edit` param) shows
   * only Back + a same-page Edit link; `?edit=1` shows the full admin action
   * set (Assign/Request Approve/Approval Decision/Convert to Lesson
   * Learned) as before. Independent of mode: once the ticket is deleted or
   * CLOSED, the footer collapses to Back only, since there's nothing left
   * to action either way. */
  managementActionsGate?: boolean;
}) {
  ...
  // Same idea, scoped to just the comment box, for the plain Tickets list.
  const showCommentForm = !commentGate || isEditMode;
  ...
  // Neither query depends on the other's result (both are keyed off the
  // route param `id`) — fetched together instead of one after the other.
  const [ticket, approval] = await Promise.all([
    ...
    prisma.ticketApproval.findUnique({
      where: { ticketId: id },
      select: {
        id: true,
        supervisorId: true,
  ...
  // Ticket Management only: nothing left to action once deleted or closed
  // (footer collapses to Back only); otherwise View mode offers just a
  // same-page Edit link, Edit mode shows the full admin action set.
  // Everywhere else this is always true (unaffected) — it's an additional
  // AND-condition alongside each button's own existing rule, not a
  // replacement for it.
  const isClosedOrDeleted = ticket.status === "CLOSED" || Boolean(ticket.deletedAt);
  const managementActionsOk = !managementActionsGate || (!isClosedOrDeleted && isEditMode);
  const showManagementEditLink = managementActionsGate && !isClosedOrDeleted && !isEditMode;
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        breadcrumbs={[{ label: defaultBackLabel, href: defaultBackHref }, { label: ticket.ticketNumber }]}
      />
            ...
            <div className="col-12">
              ...
              <div className="card card-primary card-outline mb-4">
                ...
                <div className="tab-content">
                  ...
                  <div className="tab-pane fade" id="comment-pane" role="tabpanel" aria-labelledby="comment-tab">
                    <div className="card-body">
                      ...
                      {showCommentForm && <AddCommentForm ticketId={ticket.id} />}
                    </div>
                  </div>
                ...
                <div className="card-footer d-flex gap-2 justify-content-end align-items-center">
                  {isAdmin && !assigneeActionsGate && managementActionsOk && ticket.status !== "CLOSED" && (
                    <AssignButton
                      ticketId={ticket.id}
                  ...
                  {isSupervisor && managementActionsOk && approval?.status === "PENDING" && (
                  ...
                  {isAdmin && showConvertToLessonLearned && managementActionsOk && (
                  ...
                  {canEdit && managementActionsOk && (
                    <Link href={`/tickets/${ticket.id}/edit`} className="btn btn-outline-secondary">
                      <i className="bi bi-pencil me-1" aria-hidden="true"></i>
                      Edit
                    </Link>
                  )}
                  {showManagementEditLink && (
                    // managementActionsGate is only ever true from the Ticket
                    // Management route, so this can hardcode that prefix —
                    // same precedent as assigneeActionsRedirect below.
                    <Link
                      href={`/transaction/ticket-management/${ticket.id}?edit=1${back ? `&back=${encodeURIComponent(back)}` : ""}`}
